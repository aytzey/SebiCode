...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    ...
    // Cache fix: persist deferred_tools_delta and mcp_instructions_delta so
    // resume sessions reconstruct the correct cache prefix
    if (m.attachment.type === 'deferred_tools_delta') return true
    if (m.attachment.type === 'mcp_instructions_delta') return true
    return false
  }
  return true
}
