...
/sebiralph "task description" (built-in local-jsx command)
  ...
  └─ Phase 4: Review, Merge, Deploy & Verify
      ...
      Runtime verification loop until gaps are closed
```
...
type RalphWorkflowDefaults = {
  tdd: boolean; deployVerification: boolean;
...
| `orchestrator.ts` | Main orchestration prompt with progress markers and deploy-verify loop |
