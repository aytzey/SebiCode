...
export function extractFirstMessageText(
  ...
  // Cache fix: skip synthetic meta messages so the fingerprint stays stable on resume
  const firstUserMessage = messages.find(msg => msg.type === 'user' && !('isMeta' in msg && msg.isMeta))
  if (!firstUserMessage) {
    return ''
  }
